...
def delete_test(request, pk):
    ...
    Args:
        request: HTTP
        pk: campaign_id
    Returns: delete an existing test of a given campaign
    """
    campaign = Campaign.objects.get(pk=pk)
    existing_test_id = int(request.POST.get('test_id'))
    if existing_test_id:
        campaign.tests.remove(existing_test_id)
        campaign.save()
...
def edit_test(request,pk):
    ...
    Args:
        request: HTTP
        pk: test primary key
    Returns: This controls the test in campaign dashboard
    """

    action = request.POST.get('action')
    candidate_id = request.POST.get('test_id')   
    pass
...
def new_test(request):
    ...
    qtypes_json = json.dumps(list(question_types), cls=DjangoJSONEncoder)
    question_types_json = serializers.serialize("json", question_types)
    questionsTypes = json.loads(question_types_json)

    return render(request, cts.NEW_TEST, {'question_types': question_types,
                                          'question_types_json': qtypes_json
                                          })
